...
import {
  PageData, BubbleData, ImageData,
  IncomingMessage, OutgoingMessage, NodeMapping,
...
export class SyncEngine {
  ...
  async syncPage(data: PageData): Promise<void> {
    ...
    try {
      ...
      if (!frame) {
        frame = await this.factory.createPageFrame(data.pageIndex);
      ...
      // 2) 이미지 배치/업데이트
      if (data.image) {
        await this.syncImage(frame, data.image);
      }

      // 3) 버블(대사/나레이션/SFX) 동기화 — 레이어 순서 유지
      //    나레이션 → 대사 → SFX 순으로 쌓기 (SFX가 최상단)
      const sorted = [...data.bubbles].sort((a, b) => {
  ...
  private async syncImage(frame: FrameNode, imageData: ImageData): Promise<void> {
    const hash = simpleHash(imageData.base64.substring(0, 200));
    const existing = this.findExistingNode(imageData.id);

    if (existing && existing.type === "RECTANGLE") {
      // 내용이 변경된 경우만 업데이트
      const oldHash = existing.getPluginData("contentHash");
      if (oldHash !== hash) {
        await this.factory.updateImage(existing as RectangleNode, imageData);
        existing.setPluginData("contentHash", hash);
      }
    } else {
      const node = await this.factory.placeImage(frame, imageData);
  ...
  private async syncBubble(frame: FrameNode, bubble: BubbleData): Promise<void> {
    const hash = simpleHash(JSON.stringify(bubble));
    const existing = this.findExistingNode(bubble.id);

    if (existing) {
      const oldHash = existing.getPluginData("contentHash");
      if (oldHash !== hash) {
        await this.factory.updateBubbleNode(existing, bubble);
        existing.setPluginData("contentHash", hash);
      }
    } else {
  ...
  async updateBubble(bubble: BubbleData): Promise<void> {
    ...
    await this.factory.updateBubbleNode(existing, bubble);
    existing.setPluginData("contentHash", simpleHash(JSON.stringify(bubble)));
    this.respond({ type: "SYNC_OK", id: bubble.id, figmaNodeId: existing.id });
  }
