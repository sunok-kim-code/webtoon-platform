...
2. outfit.normalized_id는 같은 의상이 여러 장면에 나오더라도 **항상 동일한 ID**로 생성한다. 형식은 소문자 + underscore만 사용 (예: casual_red_hoodie_blue_jeans, school_uniform_winter_coat).
   - 의상 주요 특징 3~5개만 압축해서 ID 생성.
3. location.canonical_category는 반드시 아래 목록 중 하나로만 선택:
...
4. 출력은 **반드시 아래 JSON 스키마 그대로만** 반환하고, panels/dialogue/narration/sound_effects 같은 추가 필드는 절대 넣지 말 것. 마크다운 코드블록도 금지.
5. 씬에 등장하는 캐릭터가 없으면 characters를 빈 배열 []로 반환.
...
function buildClaudeSceneAnalysisPrompt(
  ...
  const charContext = existingCharacters.length > 0
    ? `\n\n[기존 등록된 캐릭터 — appearance_core 참고용]\n${existingCharacters
...
async function analyzeSceneWithKieClaudeSonnet46(
  ...
  const userPrompt = buildClaudeSceneAnalysisPrompt(sceneText, sceneId, existingCharacters, existingLocations);
  ...
  try {
    ...
    return mapClaudeResultToGemini(result, sceneText);
  } catch (e: any) {
...
function mapClaudeResultToGemini(
  ...
  const characters: GeminiCharacterAnalysis[] = claude.characters.map(c => ({
    ...
    outfitLabel: c.outfit.normalized_id.replace(/_/g, " "),
  ...
  return {
    ...
    panels,
    sceneOverview: `${sceneLines.length}개 장면, ${claude.characters.length}명 등장 — ${locationName}`,
...
async function analyzeSceneWithClaudeSonnet(
  ...
  try {
    ...
    return mapClaudeResultToGemini(parsed, sceneText);
  } catch (e: any) {
...
export async function analyzeSceneWithGemini(
  ...
  if (getSelectedModel() === "kie-claude-sonnet-4-6") {
    return analyzeSceneWithKieClaudeSonnet46(sceneText, existingCharacters, existingLocations, _sceneId);
  }

  // ── Anthropic 직접 연결 — claude-sonnet-4-6 ──────────────────
  if (getSelectedModel() === "claude-sonnet-4-6") {
    return analyzeSceneWithAnthropicDirect(sceneText, existingCharacters, existingLocations, _sceneId);
  }

  // ── Claude Sonnet 4.5 via Kie.ai ─────────────────────────────
  if (getSelectedModel() === "claude-sonnet-4-5") {
    return analyzeSceneWithClaudeSonnet(sceneText, existingCharacters, existingLocations, _sceneId);
  }
