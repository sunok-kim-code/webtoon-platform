...
2. 【캐릭터 이름 규칙 — 필수】 characters[].name은 반드시 원작 **한국어(한글) 이름**으로만 표기하라.
   - 영어 로마자 표기 절대 금지. 예) "seojun" → "서준" 또는 "박서준", "eunseo" → "이은서"
   - 기존 등록된 캐릭터가 있으면 그 이름을 그대로 사용할 것.
3. 【의상 ID 규칙 — 필수】 outfit.normalized_id는 반드시 '{한국어캐릭터이름}_{의상특징}' 형식으로 생성하라.
...
4. location.canonical_category는 반드시 아래 목록 중 하나로만 선택:
